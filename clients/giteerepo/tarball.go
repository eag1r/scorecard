...
func (handler *tarballHandler) init(ctx context.Context, repo gitee.Project) error {
...
func (handler *tarballHandler) getTarball(ctx context.Context, repo gitee.Project) error {
	url := repo.HtmlUrl
	url = strings.Replace(url, "{.git}", "archive/" + repo.DefaultBranch + ".zip", 1)
