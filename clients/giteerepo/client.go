...
// Package giteerepo implements clients.RepoClient for Gitee.
package giteerepo
...
// Client is Gitee-specific implementation of RepoClient.
type Client struct {
	owner        string
	repoName     string
	repo         gitee.Project
	repoClient   *gitee.APIClient
...
// InitRepo sets up the Gitee repo in local storage for improving performance and GitHub token usage efficiency.
func (client *Client) InitRepo(inputRepo clients.Repo) error {
	...
	// Sanity check.
	repo, _, err := client.repoClient.RepositoriesApi.GetV5ReposOwnerRepo(client.ctx, ghRepo.owner, ghRepo.repo, nil)
	if err != nil {
		return sce.WithMessage(sce.ErrRepoUnreachable, err.Error())
	}
	client.repo = repo
	client.owner = repo.Owner.Login
	client.repoName = repo.Name
...
func (client *Client) URI() string {
	return fmt.Sprintf("gitee.com/%s/%s", client.owner, client.repoName)
}
...
func (client *Client) ListMergedPRs() ([]clients.PullRequest, error) {
	//return client.repoClient.ListMergedPRs(), nil
	pulls := []clients.PullRequest{}
	return pulls, nil
}

// ListCommits implements RepoClient.ListCommits.
func (client *Client) ListCommits() ([]clients.Commit, error) {
	//return client.repoClient.ListCommits(), nil
...
func (client *Client) IsArchived() (bool, error) {
	//return client.repoClient.isArchived()
	return false, nil
}
...
// CreateGiteeRepoClient returns a Client which implements RepoClient interface.
func CreateGiteeRepoClient(ctx context.Context, logger *zap.Logger) clients.RepoClient {
	//// Use our custom roundtripper
	//rt := roundtripper.NewTransport(ctx, logger.Sugar())
	//httpClient := &http.Client{
	//	Transport: rt,
	//}
	token := "3c52a378b124dd5be93c70b4006a70e0"
	...
	cfg.HTTPClient = oauth2.NewClient(context.Background(), ts)
...
func CreateOssFuzzRepoClient(ctx context.Context, logger *zap.Logger) (clients.RepoClient, error) {
	ossFuzzRepo, err := MakeGiteeRepo("https://github.com/google/oss-fuzz")
	if err != nil {
		return nil, fmt.Errorf("error during giteerepo.MakeGiteeRepo: %w", err)
	}

	ossFuzzRepoClient := CreateGiteeRepoClient(ctx, logger)
